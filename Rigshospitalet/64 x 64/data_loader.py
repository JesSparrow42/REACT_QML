...
def to_dcm(ct_tensor, output_dir, pixel_spacing=(1.0, 1.0), slice_thickness=1.0):
    """
    Converts a 3D tensor representing a CT scan into a series of DICOM files. NOT WORKING.
    ...
    ct_array = ct_tensor.cpu().numpy()  # Convert to NumPy array (Metal Perfomance Shader req. moved to CPU)
...
class DICOMDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        ct_image_tensor = torch.tensor(ct_image).unsqueeze(0)  # [1, H, W]
        pet_image_tensor = torch.tensor(pet_image).unsqueeze(0)  # [1, H, W]

        if self.augment:
            ct_image_tensor = self.transform(ct_image_tensor)
            pet_image_tensor = self.transform(pet_image_tensor)
